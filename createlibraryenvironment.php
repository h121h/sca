<?php
//Usage : curl -X POST -k -u username:api_key 'http://169.44.167.82/createlibraryenvironment.php' --data '{"location":"<location>", "environment_id": "XX", 
//																											"name":"<library env name>", "description":"<description>"}'
...
// Get UserID
	...
	logit("SQL","$ssql");
	$results=try_conn($conn, $ssql, "", "Failed to find user id");
...
// Log the request
	$ssql = "INSERT into requests (json_in, user_id, function, status)
			 VALUES ('{$json}', '{$user_id}', 'createlibraryenvironment', 'new')";
	logit("SQL","$ssql");
...
//Get environment id, name, and description from JSON and validate from DB
	...
	$library_name = check_array($json_array,"name");
	$library_description = check_array($json_array,"description");
...
// Environment name : check if the name is blank or is a duplicate
	if(strlen($library_name) < 1) {
		$json_errors[$error_index++] = "Missing library name";
	}
	if($environment_id =="") {
		$json_errors[$error_index++] = "Missing environment id";
	}
...
// Direct the call to the proper engine
	...
	$cmdd = "curl -X POST -k -u '{$username}:{$api_key}' 'http://{$engine_ip}:{$engine_port}/taskcreatelibraryenvironment.php' --data '{$json_sent}'";
	$cmdd_obscured = str_replace($api_key, "<api_key>", $cmdd);
	logit("CMD", "Create library environment command: {$cmdd_obscured}");
...
// Log and exit
	...
	echo $myJSON;
	logit("INFO","createlibraryenvironment API completed");
?>