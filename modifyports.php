<?php
// Usage: curl -X POST -k -u username:api_key 'https://api.slayercloud.com/modifyports.php?test=yes' --data '{"location":"<location>", "environment_id" : "<ENVIRONMENT_ID>", "node_id" : "<NODE_ID>", "add" : [ "<PORT>", "<PORT>", "<PORT>" ], "remove" : [ "<PORT>", "<PORT>", "<PORT>" ]}'
...
// Log the request
	$ssql = "INSERT into requests (json_in, user_id, function, status)
			 VALUES ('{$json}', '{$user_id}', 'modifyports', 'new')";
	logit("SQL","$ssql");
...
//Get environment id, name, and description from JSON and validate from DB
	...
	$remove = check_array($json_array,"remove");
	$add = check_array($json_array,"add");
...
// Library and environment name : check if the name is blank
	if($environment_id == "") {
		$json_errors[$error_index++] = "Missing library id";
	}
	
	if($node_id == "") {
		$json_errors[$error_index++] = "Missing node id";
	}
	
	if($add == "" and $remove == "") {
		$json_errors[$error_index++] = "No port changes found";
	}	
...
// Direct the call to the proper engine
	...
	$cmdd = "curl -X POST -k -u '{$username}:{$api_key}' 'http://{$engine_ip}:{$engine_port}/modifyports.php' --data '{$json_sent}'";
	$cmdd_obscured = str_replace($api_key, "<api_key>", $cmdd);
	logit("CMD", "Modify port command: {$cmdd_obscured}");
...
// Log and exit
	...
	echo $myJSON;
	logit("INFO","modifyports API completed");
?>