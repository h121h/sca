<?php
//Usage : curl -X POST -k -u username:api_key 'http://169.44.167.82/modifyenvironment.php' --data '{"location":"<location>", "action": "<add or delete>", "environment_id": "XX", rest of json}'
...
// Log the request
	$ssql = "INSERT into requests (json_in, user_id, function, status)
			 VALUES ('{$json}', '{$user_id}', 'modifyenvironment', 'new')";
	logit("SQL",$ssql);
...
// Library and environment name : check if the name is blank
	if($action <> "add" and $action <> "delete") {
		$json_errors[$error_index++] = "Invalid action";
	}
...
// Direct the call to the proper engine
	...
	if($action == "add") {
		$function_name = "addtoenvironment";
	}
	if($action == "delete") {
		$function_name = "taskdeletefromenvironment";
	}	
	$cmdd = "curl -X POST -k -u '{$username}:{$api_key}' 'http://{$engine_ip}:{$engine_port}/{$function_name}.php' --data '{$json_sent}'";
	logit("CMD", "Modify environment command: {$cmdd}");
...
// Log and exit
	...
	echo $myJSON;
	logit("INFO","modifyenvironment API completed");
?>