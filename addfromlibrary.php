<?php
//Usage : curl -X POST -k -u username:api_key 'http://169.44.167.82/addfromlibrary.php' --data '{"location":"<location>", "library_id": "XX", "environment_id": "XX"}'
...
// Log the request
	$ssql = "INSERT into requests (json_in, user_id, function, status)
			 VALUES ('{$json}', '{$user_id}', 'addfromlibrary', 'new')";
	logit("SQL","$ssql");
...
// Direct the call to the proper engine
	...
	$cmdd = "curl -X POST -k -u '{$username}:{$api_key}' 'http://{$engine_ip}:{$engine_port}/addfromlibrary.php' --data '{$json_sent}'";
	$cmdd_obscured = str_replace($api_key, "<api_key>", $cmdd);
	logit("CMD", "Add to environment command: {$cmdd_obscured}");
...
// Log and exit
	...
	echo $myJSON;
	logit("INFO","addfromlibrary API completed");
?>