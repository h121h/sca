<?php
// Usage: curl -X POST -k -u username:api_key 'https://api.slayercloud.com/add_ftp_ports.php?test=yes' --data '{"location":"<location>", "environment_id" : "<ENVIRONMENT_ID>", "node_id" : "<NODE_ID>", "count" : "<count>"}'
...
//Get environment id, name, and description from JSON and validate from DB
	...
	$count = check_array($json_array,"count");
...
// Library and environment name : check if the name is blank
	...
	if($count == "") {
		$json_errors[$error_index++] = "No port change found";
	}	
...
// Direct the call to the proper engine
	...
	$cmdd = "curl -X POST -k -u '{$username}:{$api_key}' 'http://{$engine_ip}:{$engine_port}/add_ftp_ports.php' --data '{$json_sent}'";
	$cmdd_obscured = str_replace($api_key, "<api_key>", $cmdd);
	logit("CMD", "Add ftp ports command: {$cmdd_obscured}");
