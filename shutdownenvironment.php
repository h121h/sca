...
//Usage : curl -X POST -k -u username:api_key 'https://api.slayercloud.com/shutdownenvironment.php' --data '{"location":"<location>", "environment_id": "XX"}'
...
// Log the request
	$ssql = "INSERT into requests (json_in, user_id, function, status)
			 VALUES ('{$json}', '{$user_id}', 'deleteenvironment', status = 'new')";
	logit("SQL","$ssql");
...
// Direct the call to the proper engine
	...
	$cmdd = "curl -X POST -k -u '{$username}:{$api_key}' 'http://{$engine_ip}:{$engine_port}/shutdownenvironment.php' --data '{$json_sent}'";
	$cmdd_obscured = str_replace($api_key, "<api_key>", $cmdd);
	logit("CMD", "Shut down environment command: {$cmdd_obscured}");
...
// Log and exit
	...
	echo $myJSON;
	logit("INFO","shutdownenvironment API completed");
?>